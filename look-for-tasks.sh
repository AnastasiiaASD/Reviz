...
OUTPUT=$(jira-ai run-jql "assignee = currentUser() AND status = 'Prep Autotests' AND (labels IS EMPTY OR (labels != analyzed AND labels != qaed))" --limit 1)
...
if [ -n "$TASK_ID" ]; then
    ...
    if [ -n "$TASK_ID" ]; then
        ...
        OUTPUT=$(jira-ai run-jql "assignee = currentUser() AND status = 'Prep Autotests' AND (labels IS EMPTY OR (labels = analyzed AND labels = qaed AND labels != pr_created))" --limit 1)
        TASK_ID=$(echo "$OUTPUT" | grep "│" | grep -v "Key" | awk -F '│' '{print $2}' | tr -d '[:space:]')
