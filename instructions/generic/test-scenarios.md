...
- Process scenarios **one by one**. For each scenario:
    ...
    3. **Save Immediately:** Append these technical details to `/app/tmp/{{task-key}}_scenarios.md` before starting the next scenario. Ensure this data is clear enough to be used directly for autotest creation.
