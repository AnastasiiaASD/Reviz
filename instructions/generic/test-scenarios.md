...
- Process scenarios **one by one**. For each scenario:
    ...
    3. **Save Immediately:** Append these technical details to the corresponding `/app/tmp/{{task-key}}_comment.md` file before starting the next scenario. Ensure this data is clear enough to be used directly for autotest creation.
...
### 3. Jira Reporting
- Post a comment on the Jira from the file that you have created.
  ```bash
  jira-ai add-comment --file-path /app/tmp/{{task-key}}_comment.md --issue-key {{task-key}}
  ```

### 4. Add "qaed" label to task
```bash
